...
(defun ask-user-about-supersession-threat (filename)
  ...
  (assert (eq (buffer-file-name) filename))
  (message "Reverting file %s..." filename)
  (revert-buffer t t)
  (message "Reverting file %s... done" filename))
;; Save a copy of the buffer before reverting
(add-hook
 'before-revert-hook
 (lambda () (write-region nil nil (concat buffer-file-name "~reverted~")))
 nil nil)
...
(autoload 'tac-mode "tac-mode" "Major mode for editing TAC files." t)
